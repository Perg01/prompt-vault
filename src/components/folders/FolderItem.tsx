...
import { renameFolder } from "@/src/lib/actions/folder.actions";
...
export default function FolderItem({ folder }: FolderItemProps) {
  ...
  return (
    <li className="p-3 border rounded-lg shadow-sm hover:bg-gray-600 flex justify-between items-center">
      {isEditing ? (
        <input
          ref={inputRef}
          type="text"
      ...
      <div className="space-x-2 flex items-center">
        {isEditing ? (
          ...
          >
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className="size-5 text-blue-500 hover:text-blue-700"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
        ...
        {/* Delete Button is next */}
        <button className="p-1" aria-label={`Delete folder ${folder.name}`}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
